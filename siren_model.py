...
class SIREN(nn.Module):
    ...
    def __init__(self, n_in_features, n_out_features, hidden_layer_config=[],
        ...
        #     spectral_norm(nn.Linear(n_in_features, hidden_layer_config[0])),
        #     SineLayer(self.w0)
        # ))
        #
        # for i in range(1, len(hidden_layer_config)):
        #     net.append(nn.Sequential(
        #         spectral_norm(nn.Linear(hidden_layer_config[i - 1], hidden_layer_config[i])),
        #         SineLayer(self.ww)
        #     ))
        #
        # net.append(nn.Sequential(
        #     spectral_norm(nn.Linear(hidden_layer_config[-1], n_out_features)),
        # ))
